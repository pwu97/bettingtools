...
nba_2019_2020_data_2019 <- read_csv("~/Downloads/nba_odds_2019.csv") %>%
...
nba_2019_2020_data_2020 <- read_csv("~/Downloads/nba_odds_2019.csv") %>%
  left_join(nba_teams, by = "Team") %>%
  mutate(sportslogos_name = ifelse((is.na(sportslogos_name) & Team == "LALakers"),
                                   "Los Angeles Lakers", sportslogos_name),
         sportslogos_name = ifelse((is.na(sportslogos_name) & Team == "LAClippers"),
...
rbind(nba_2019_2020_data_2019, nba_2019_2020_data_2020) %>%
  clean_names() %>%
  group_by(date, game) %>%
